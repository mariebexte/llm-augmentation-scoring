...
SCORING_PROMPT = """
<purpose>
You are a school teacher and need to grade your student's exams.
This is one of the exam questions: 
{question}
This the reference answer for this question:
{ref_answer}
This is the answer the student gave:
{answer}
Decide which of these five labels best describes the student's answer:
- Correct: It is a correct answer to the question.
- Partially correct or incomplete: It contains some but not all necessary information from the reference answer.
- Contradictory: It contradicts the reference answer.
- Irrelevant: It is on topic, but does not answer the question.
- Non-domain: It is off-topic, e.g., "I don't know", "what the book says", "you are stupid".
</purpose>
<format_rules>
Only output the label you want to assign.
</format_rules>
<output>
Compare the student's answer to the reference answer and decice which label best describes it.
</output>
"""
...
# This is the answer the student gave:
# {answer}
# You now have to label the student answer.
# These are the possible labels:
# - Correct: The answer is correct.
# - Partially correct or incomplete: The answer is a partially correct and contains some but not all necessary information.
# - Contradictory: The answer explicitly contradicts the correct answer.
# - Irrelevant: The answer is talking about domain content, but does not answer the question.
# - Non-domain: The answer does not include domain content, e.g., "I don't know", "what the book says", "you are stupid".
# </purpose>
# <format_rules>
# Only output the score.
# </format_rules>
# <output>
# Decide which of the following labels is appropriate for the student answer: correct, partially correct or incomplete, contradictory, irrelevant, non-domain
# </output>
# """
...
def score_prompt(df, answer_column, target_column, question, ref_answer, target_path):
    ...
    return get_macro_f1(y_true=df[target_column], y_pred=preds)



# results_dir = 'llm_preds_v3_1'
...
df_questions = pd.read_csv('data/prompts_without_pictures.csv').set_index('id')
results_dir = 'llm_preds_vs_cleaned'
prompt_performances = {}
...
# Go over each prompt
data_source = 'data/generated_llm_answers'

for prompt in ['ME_27b_llm.csv', 'PS_4bp_llm.csv', 'VB_1_llm.csv']:

    prompt_name = prompt[:prompt.rindex('_')]
    ...
    if prompt_name in df_questions.index:
        ...
        df = pd.read_csv(os.path.join(data_source, prompt))
        prompt_f1 = score_prompt(df=df, answer_column='text_clean', target_column='label', question=question, ref_answer=ref_answers[prompt_name], target_path=os.path.join(results_dir, prompt_name + '_pred_deepseek.csv'))
...
#             df = pd.read_csv(os.path.join('llm_preds_' + str(run), prompt + '_pred_deepseek.csv'))
...
#         prompt_performances[prompt] = {'voted': get_macro_f1(y_true=df_full['Score'], y_pred=df_full['pred_deepseek_voted']),
#                                        'run_1': get_macro_f1(y_true=df_full['Score'], y_pred=df_full['pred_deepseek_1']),
#                                        'run_2': get_macro_f1(y_true=df_full['Score'], y_pred=df_full['pred_deepseek_2']),
#                                        'run_3': get_macro_f1(y_true=df_full['Score'], y_pred=df_full['pred_deepseek_3']),
#                                        'run_4': get_macro_f1(y_true=df_full['Score'], y_pred=df_full['pred_deepseek_4']),
#                                        'run_5': get_macro_f1(y_true=df_full['Score'], y_pred=df_full['pred_deepseek_5']),
#                                        }
