    ...
    for task in ['ME_27b_llm.csv', 'PS_4bp_llm.csv', 'VB_1_llm.csv']:
        ...
        if task.endswith('.csv'):
            ...
            for test_idx, df_test_orig in df_test.iterrows():
                ...
                if method == 'LR':
                    pred_orig = pred_orig + list(train_shallow(method=method, df_train=df_train_orig, df_test=df_test_orig, answer_column='AnswerText', target_column=target_column_test))
            ...
            current_results = {'LOOCV_orig': calculate_macro_f1(y_true=true_scores, y_pred=pred_orig), 'LLM_labels_full_clean': calculate_macro_f1(y_true=true_scores, y_pred=pred_full)}
